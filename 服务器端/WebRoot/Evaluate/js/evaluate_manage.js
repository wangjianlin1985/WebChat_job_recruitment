...
function initEvaluateManageTool() {
	evaluate_manage_tool = {
		init: function() {
			$.ajax({
				...
				success : function (data, response, status) {
					$("#sellerObj_sellerUserName_query").combobox({ 
					    valueField:"sellerUserName",
					    textField:"sellerName",
					    panelHeight: "200px",
				        editable: false, //不允许手动输入 
					});
					data.splice(0,0,{sellerUserName:"",sellerName:"不限制"});
					$("#sellerObj_sellerUserName_query").combobox("loadData",data); 
				}
			});
			$.ajax({
		...
		search: function() {
			...
			queryParams["sellerObj.sellerUserName"] = $("#sellerObj_sellerUserName_query").combobox("getValue");
			queryParams["evaluateContent"] = $("#evaluateContent").val();
