...
public class EvaluateController extends BaseController {
	...
	/*ajax方式按照查询条件分页查询商家评价信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(@ModelAttribute("sellerObj") Seller sellerObj,String evaluateContent,@ModelAttribute("userObj") UserInfo userObj,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
	...
	public void listAll(HttpServletResponse response) throws Exception {
		...
		for(Evaluate evaluate:evaluateList) {
			...
			jsonEvaluate.accumulate("evaluateContent", evaluate.getEvaluateContent());
			jsonArray.put(jsonEvaluate);
		}
	...
	public String frontlist(@ModelAttribute("sellerObj") Seller sellerObj,String evaluateContent,@ModelAttribute("userObj") UserInfo userObj,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (evaluateContent == null) evaluateContent = "";
		List<Evaluate> evaluateList = evaluateService.queryEvaluate(sellerObj, evaluateContent, userObj, currentPage);
	    /*计算总的页数和总的记录数*/
	    evaluateService.queryTotalPageAndRecordNumber(sellerObj, evaluateContent, userObj);
	    /*获取到总的页码数目*/
	    int totalPage = evaluateService.getTotalPage();
	    ...
	    request.setAttribute("sellerObj", sellerObj);
	    request.setAttribute("evaluateContent", evaluateContent);
	...
	public String delete(@PathVariable Integer evaluateId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  evaluateService.deleteEvaluate(evaluateId);
	            request.setAttribute("message", "商家评价删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "商家评价删除失败!");
				return "error";
	...
	/*按照查询条件导出商家评价信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(@ModelAttribute("sellerObj") Seller sellerObj,String evaluateContent,@ModelAttribute("userObj") UserInfo userObj, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(evaluateContent == null) evaluateContent = "";
        List<Evaluate> evaluateList = evaluateService.queryEvaluate(sellerObj,evaluateContent,userObj);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Evaluate信息记录"; 
        String[] headers = { "评价id","被评价商家","评分","评价内容","评论用户"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<evaluateList.size();i++) {
        	Evaluate evaluate = evaluateList.get(i); 
        	dataset.add(new String[]{evaluate.getEvaluateId() + "",evaluate.getSellerObj().getSellerName(),evaluate.getEvaluateScore() + "",evaluate.getEvaluateContent(),evaluate.getUserObj().getName()});
        }
        /*
