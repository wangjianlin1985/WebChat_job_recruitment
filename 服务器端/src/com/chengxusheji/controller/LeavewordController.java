...
public class LeavewordController extends BaseController {
	...
	/*ajax方式按照查询条件分页查询客服留言信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(String leaveTitle,@ModelAttribute("userObj") UserInfo userObj,String leaveTime,String replyPerson,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (leaveTitle == null) leaveTitle = "";
		if (leaveTime == null) leaveTime = "";
		if (replyPerson == null) replyPerson = "";
		if(rows != 0)leavewordService.setRows(rows);
		List<Leaveword> leavewordList = leavewordService.queryLeaveword(leaveTitle, userObj, leaveTime, replyPerson, page);
	    /*计算总的页数和总的记录数*/
	    leavewordService.queryTotalPageAndRecordNumber(leaveTitle, userObj, leaveTime, replyPerson);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String leaveTitle,@ModelAttribute("userObj") UserInfo userObj,String leaveTime,String replyPerson,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (leaveTitle == null) leaveTitle = "";
		if (leaveTime == null) leaveTime = "";
		if (replyPerson == null) replyPerson = "";
		List<Leaveword> leavewordList = leavewordService.queryLeaveword(leaveTitle, userObj, leaveTime, replyPerson, currentPage);
	    /*计算总的页数和总的记录数*/
	    leavewordService.queryTotalPageAndRecordNumber(leaveTitle, userObj, leaveTime, replyPerson);
	    /*获取到总的页码数目*/
	    int totalPage = leavewordService.getTotalPage();
	    ...
	    request.setAttribute("replyPerson", replyPerson);
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
	...
	public void update(@Validated Leaveword leaveword, BindingResult br,
		...
		leaveword.setReplyPerson(session.getAttribute("username").toString());
		try {
	...
	public String delete(@PathVariable Integer leaveWordId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  leavewordService.deleteLeaveword(leaveWordId);
	            request.setAttribute("message", "客服留言删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "客服留言删除失败!");
				return "error";
	...
	/*按照查询条件导出客服留言信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(String leaveTitle,@ModelAttribute("userObj") UserInfo userObj,String leaveTime,String replyPerson, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(leaveTitle == null) leaveTitle = "";
        if(leaveTime == null) leaveTime = "";
        if(replyPerson == null) replyPerson = "";
        List<Leaveword> leavewordList = leavewordService.queryLeaveword(leaveTitle,userObj,leaveTime,replyPerson);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Leaveword信息记录"; 
        String[] headers = { "留言id","留言标题","留言人","留言时间","回复内容","回复人","回复时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<leavewordList.size();i++) {
        	Leaveword leaveword = leavewordList.get(i); 
        	dataset.add(new String[]{leaveword.getLeaveWordId() + "",leaveword.getLeaveTitle(),leaveword.getUserObj().getName(),leaveword.getLeaveTime(),leaveword.getReplyContent(),leaveword.getReplyPerson(),leaveword.getReplyTime()});
        }
        /*
