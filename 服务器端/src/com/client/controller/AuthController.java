...
public class AuthController {
	...
	public JsonResult register(String avatarUrl,String genDer,String nickName,String tel,String jsCode,String role,HttpServletRequest request) throws Exception {
		...
		if(role.equals("user")) {
			...
			seller.setSellerUserName(tel);
			seller.setPassword("--"); 
			seller.setSellerName(nickName);
			seller.setSellerPhoto("upload/" + fileName);
			seller.setWorkContent("--");
			seller.setWorkTime("--");
			seller.setTelephone("--");
			seller.setWorkAddress("--");
			seller.setWorkLicense("upload/NoImage.jpg");
			seller.setScoreValue(0.0f);
			seller.setShenHeState("待审核");
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			seller.setAddTime(sdf.format(new java.util.Date())); 
			sellerService.addSeller(seller);
			sellerService.updateSellerOpenid(seller.getSellerUserName(), openId); 
 
			userToken.setRole("seller");
			userToken.setUserName(seller.getSellerUserName());
		}
		...
		resultMap.put("role", userToken.getRole()); 
	...
	public JsonResult login(WeixinLoginParams params) throws Exception {
		try {
			...
			resultMap.put(SessionConsts.AUTH_TOKEN_KEY, authToken);
			resultMap.put("role", userToken.getRole()); 
			if(userToken.getRole().equals("user")) {
				resultMap.put("avatarUrl", userInfo.getUserPhotoUrl());
				resultMap.put("nickName", userInfo.getName());
			} else {
				resultMap.put("avatarUrl", seller.getSellerPhotoUrl());
				resultMap.put("nickName", seller.getSellerName());
			} 
